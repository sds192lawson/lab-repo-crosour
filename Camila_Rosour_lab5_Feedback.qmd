...
total_stops_age_recorded <-
  ...
  summarize(Count = n()) |>
  pull()
##TA Feedback: It looks like you have the first part down just add in part 2, you can pull it directly from your answer to question 5! +2 for attempt
##Total for Q1-Q6: 17 pts
##Part 2 Code
#sqf_2011 |>
  #filter(age >= 14 & age <= 24) |>
  #summarize(total_14_24 = n(), 
...
```{r, eval = FALSE}
# Write code here.
total_stops <- sqf_2011 |>
  summarize(count = n()) |>
  pull()

q9 <- sqf_2011 |>
  group_by(race_cat) |>
  summarize(stops = n(), percent_stops = stops / total_stops * 100, percent_frisked = mean(frisked == 1) * 100, percent_wpnfound = mean(wpnfound == 1) * 100, percent_arrestsumm = mean(arrestsumm ==1))
##TA Feedback: This looks wondrous, just remember to ad na.rm = TRUE to each next time. +2.5
...
```{r, eval = FALSE}
# The summary statistics show that people who are black are stopped more frequently than any other race category (at over 50%), and by race category, white people who were stopped had the lowest percentage of frisks (44%) while black and black-Hispanics had the highest. For each race category, only between 0 and 2 percent of stops led to a weapon being found or an arrest being made. The 2011 stop and frisk activity was very racialized and did not provide any legitimacy in preventing crime; it was mainly an unconstitutional violation of people's privacy, mainly affecting black and Hispanic people of color. 
##TA Feedback: Nice answer! +3
##Total for Q1-Q10: 28.5 pts
```
...
##TA Grading: Final Score: 28.5/30 or 95%
