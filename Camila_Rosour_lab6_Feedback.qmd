...
pit_pivoted <- pit |> pivot_longer(cols = c(-1, -2),
                                   names_to = "Measure",
                                   values_to = "Value")
##TA Feedback: This looks pretty good remember you can use an index arguement to leave out just one column instead of the cols arguement. +2
#correct pivot arguements
#pivot_longer(2:117, 
               #names_to = "Measure", 
               #values_to = "Value")
##Total for Q1: 2 pts
...
pit_separate_1 <- pit_pivoted |> separate(Measure, c("Shel_Unshel", "Demographic"),
                                          sep = " - ")
...
pit_separate_2 <- pit_separate_1 |> separate(Demographic,
                                             c("Demographic", "Year"),
                                             sep = ", ")
...
# Step 1
gender_pivoted_longer <- gender |> pivot_longer(cols = c(estimate_2015, moe_2020),
                                                names_to = "Measure",
                                                values_to = "Values")

# Step 2
gender_separated <- gender_pivoted_longer |> separate(Measure,
                                               c("Measure", "Year"),
                                               sep = "_")
...
# Step 1
race_pivoted_longer <- race |> pivot_longer(cols = c(estimate_2015, moe_2020),
                                                names_to = "Measure",
                                                values_to = "Values")

# Step 2
race_separated <- race_pivoted_longer |> separate(Measure,
                                               c("Measure", "Year"),
                                               sep = "_")
...
pit_gender <- pit_by_gender |>
  left_join(gender_final, by = c("State" = "NAME", "Demographic" = "variable", "Year" = "Year"))

pit_race <- pit_by_race |>
  left_join(race_final, by = c("State" = "NAME", "Demographic" = "variable", "Year" = "Year"))
##TA Feedback: This looks tremendous! +4
...
pit_gender_rates <- pit_gender |>
  mutate(homeless_rate = 1000 * pit_gender$`Sheltered Total Homeless - Female, 2015` / Value )
pit_race_rates <- pit_race |>
  mutate(homeless_rate = 1000 *pit_gender$`Sheltered Total Homeless - Female, 2015` / Value )
##TA Feedback: This looks okay, remember that you want to divide the value of the homeless count by the estimate of the population not the other way around! Also try to multiply by 10,000 at the end, not 1,000 at the beginning! +3
#Correct Code
pit_gender_rates2 <- 
  pit_gender |>
