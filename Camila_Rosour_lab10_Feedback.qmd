...
#| eval: false
library(tidycensus)
census_api_key("fe6188cf07e417e3505d37a1d98d14b178477996", overwrite = TRUE, install = TRUE)
```
...
census_counties <- st_transform(census_counties, CR = 4326)

census_tracts <- st_transform(census_tracts, CR = 4326)
...
census_counties <- census_counties |>
  mutate(blk_per = value / summary_value * 100)
...
pal_bin_census_county_perc <- colorBin(palette="Blues", 
                    domain = census_counties$blk_per,
                    bins = 5)
...
la_map %>%
  addPolygons(data = census_counties, 
              weight = 1,
              color = "black",
              fillColor = ~pal_bin_census_county_perc(blk_per),
              fillOpacity = 0.5,) %>%
  addLegend(data = census_counties,
            title = "% Black Residents, Counties", 
...
la_map %>%
  ...
  addPolygons(data = census_tracts,
              weight = 1,
              color = "black",
              fillColor = ~pal_bin_census_tract_perc(blk_per),
              fillOpacity = 0.5,) |>
  addLegend(data = census_tracts,
            title = "% Black Residents, Tracts", 
...
##TA feedback: please come back and answer the question when you get a chance +0
...
##TA feedback: Please come back and answer this question when you get a chance! +0
##Total for Q1-Q11: 24 pts
