...
la_map <- leaflet(width = "100%") %>%
  setView(lat = 30.5191, lng = -91.40, zoom = 7) %>%
  addProviderTiles(providers$OpenStreetMap)
...
tri_la_2023 <- tri_la_2023 %>%
  ...
  st_transform(4326)
##TA Feedback: Everything looks good here! +2
...
```{r}
la_map %>%
  addCircleMarkers(data = tri_la_2023, radius = 3, color = "black", weight = 1,  fillColor = "white", fillOpacity = 40)
...
```{r}
la_map %>%
  addCircleMarkers(data = tri_la_2023, radius = ~X107..TOTAL.RELEASES/1000000, color = "black", weight = 1,  fillColor = "white", fillOpacity = 40)
...
##TA Feedback: Try using unique like this next time, otherwise this looks nice! +2.5
...
```{r}
la_map %>%
  addCircleMarkers(data = tri_la_2023, radius = ~X107..TOTAL.RELEASES/1000000, color = "black", weight = 1,  fillColor = ~pal_fact(X23..INDUSTRY.SECTOR), fillOpacity = 40) |>
  addLegend(data = tri_la_2023,
            pal = pal_fact,
            values = ~X23..INDUSTRY.SECTOR)
##TA Feedback: This looks beautiful nice job! +3
...
la_map %>%
  addCircleMarkers(data = tri_la_2023, radius = ~X107..TOTAL.RELEASES/1000000, color = "black", weight = 1,  fillColor = ~pal_num(X107..TOTAL.RELEASES), fillOpacity = 40) |>
  addLegend(data = tri_la_2023,
            pal = pal_num,
...
la_map %>%
  addCircleMarkers(data = tri_la_2023, radius = ~X107..TOTAL.RELEASES/1000000, color = "black", weight = 1,  fillColor = ~pal_bin(X107..TOTAL.RELEASES), fillOpacity = 40) |>
  addLegend(data = tri_la_2023,
            pal = pal_bin,
            values = ~X107..TOTAL.RELEASES)
...
la_map %>%
  addCircleMarkers(data = tri_la_2023, radius = ~X107..TOTAL.RELEASES/1000000, color = "black", weight = 1,  fillColor = ~pal_quant(X107..TOTAL.RELEASES), fillOpacity = 40) |>
  addLegend(data = tri_la_2023,
            pal = pal_quant,
            values = ~X107..TOTAL.RELEASES)
##TA Feedback: These look marvelous! +3
##Total for Q1-Q9: 24.5
```
...
Pal_num creates a gradient based on the number of total releases. Pal_bin gives each cirle marker a color based on a proportion. Pal_quant gives each cirle marker a color based on the range of the number of total releases. Pal_quant gives the best distribution of values.
##TA Feedback: Nice answers! +3
##Total for Q1-Q10: 27.5
...
la_map %>%
  addCircleMarkers(data = tri_la_2023, radius = ~X107..TOTAL.RELEASES/1000000, color = "black", weight = 1,  fillColor = ~pal_quant(X107..TOTAL.RELEASES), fillOpacity = 40,
                   popup = ~paste("<b>", X4..FACILITY.NAME, "</b><br><i>", X23..INDUSTRY.SECTOR, "</i>")) |>
                   popupOptions(maxHeight = 200,
                         closeOnClick = TRUE)
  addLegend(data = tri_la_2023,
            pal = pal_quant,
            values = ~X107..TOTAL.RELEASES)
  ##TA Feedback: This looks terrific! +3
  ##Total for Q1-Q11: 30.5
```
