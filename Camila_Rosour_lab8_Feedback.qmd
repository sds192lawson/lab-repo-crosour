...
open_payments_dates_cleaned <- open_payments_original |>
  select(Date_of_Payment, Payment_Publication_Date) |>
...
open_payments_dates_cleaned |>
  ...
  map(is.Date)
##TA Feedback: Use map_lgl() here instead of map()! +2.5
##Total for Q1-Q2: 5.5 pts
...
open_payments <- open_payments_original |>
...
```
```{r}
#rm(open_payments_original, open_payments_dates_cleaned) ##TA Feedback: you can leave these datasets you might need them later!
...
top_10_doctors <- open_payments |>
  ...
  filter(lubridate::year(Date_of_Payment) == 2024) |>
  group_by(Covered_Recipient_NPI) |>
  summarize(sum_total_payments = sum(Total_Amount_of_Payment_USDollars, na.rm = TRUE)) |>
  arrange(desc(sum_total_payments)) |>
  select(Covered_Recipient_NPI, sum_total_payments) |>
  head(10)
...
get_doctor_name <- function(doctor_id){
  open_payments |>
    ...
    summarize(Covered_Recipient_Full_Name = first(Covered_Recipient_Full_Name)) |>
...
top_10_doctors_names <- top_10_doctors_ids |>
...
calculate_payment_type_amts <- function(doctor_id) {
  open_payments |>
    ...
    summarize(
      num_payments = sum(Number_of_Payments_Included_in_Total_Amount, na.rm = TRUE),
      total_payments = sum(Total_Amount_of_Payment_USDollars, na.rm = TRUE),
      .groups = "drop")
}

# Iterate calculate_payment_type_amts over top_10_doctors_ids and set names to top_10_doctors_names

payment_type_amts_list <- top_10_doctors_ids |>
  map(calculate_payment_type_amts) |>
...
payments_calendar <- function(doctor_id, doctor_name) {
  open_payments |>
    filter(Covered_Recipient_NPI == doctor_id) |>
    ##TA Feedback: You need a groupby() and summarize() here not ggplot
    ##TA Feedback: Like this, Correct Code:  group_by(Nature_of_Payment_or_Transfer_of_Value) |>
    ...
    ##your code:
    #ggplot(aes(
     # x = lubridate::day(Date_of_Payment),
      #y = "")) +
    #geom_point() +
    #facet_wrap(~ lubridate::month(Date_of_Payment, label = TRUE)) +
   # labs(
     # title = doctor_name,
     # x = "Day of Payment",
     # y = "")
...
payment_calendar_plots <- map2(
  top_10_doctors_ids,
  top_10_doctors_names,
  payments_calendar)
##TA Feedback: Nice job here though! +2
##Total for Q1-Q9: 25.5 pts
```
...
manufacturer_payments_df <- top_10_doctors_ids |>
  map_df(calculate_manufacturer_payments)
...
ggplot(manufacturer_payments_df, aes(
  ...
  labs(
    x = "Manufacturer",
    y = "Total Payments",
    title = "Payments for Top 10 Doctors")
